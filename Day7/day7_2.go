...
func main() {
	...
	freeSpace := 70000000 - newDirTree["/"]
	fmt.Printf("Free Space: %d\n", freeSpace)

	spaceNeeded := 30000000 - freeSpace
	fmt.Printf("Size Needed to clear: %d\n", spaceNeeded)

	toDel := make(map[string]int)
	for ndir, nsize := range newDirTree {
	...
	lowSize := 99999999999
	for _, size := range toDel {
		if size < lowSize {
			lowSize = size
		}
	}

	fmt.Printf("\n\nSize of smallest folder: %d\n", lowSize)
}
